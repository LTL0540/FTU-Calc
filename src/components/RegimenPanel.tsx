  ...
  return (
    <section className="card section-card schedule-panel">
      ...
      <div className="regimen-primary">
        ...
        <label><span>Duration</span><div className="duration-combo"><input aria-label="Treatment duration in whole units" type="number" min="1" step="1" value={props.durationValue} onChange={(event) => props.onDurationValueChange(Math.max(1, Math.round(Number(event.target.value))))} /><select aria-label="Duration unit" value={props.durationUnit} onChange={(event) => props.onDurationUnitChange(event.target.value as DurationUnit)}><option value="days">Days</option><option value="weeks">Weeks</option><option value="months">Months</option></select></div></label>
      </div>
