  ...
  return (
    <section className="card section-card schedule-panel">
      ...
      <div className="regimen-primary">
        ...
        <label><span>Duration</span><div className="duration-combo"><input aria-label="Treatment duration in whole units" type="number" min="1" step="1" placeholder="Enter" value={props.durationValue > 0 ? props.durationValue : ''} onChange={(event) => { const next = event.target.value; props.onDurationValueChange(next === '' ? 0 : Math.max(1, Math.round(Number(next)))); }} /><select aria-label="Duration unit" value={props.durationUnit} onChange={(event) => props.onDurationUnitChange(event.target.value as DurationUnit)}><option value="days">Days</option><option value="weeks">Weeks</option><option value="months">Months</option></select></div></label>
      </div>
